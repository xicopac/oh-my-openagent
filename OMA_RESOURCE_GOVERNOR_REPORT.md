...
workspace's user enable/disable state (the permission boundary), carries full price dimensions and
capability/context metadata per candidate, and can filter by required capability (vision, tool call,
reasoning, min context) rather than price alone. Disabled models never enter circulation, are never
re-enabled, and are never routed around through an alias or a different provider.

### Authoritative model / price source

- Models + capabilities + context: OpenCode live `client.model.list()` (per-model `capabilities`,
  `limit.context`) and the connected provider-models cache (`~/.omo/.../provider-models.json`
  `ModelMetadata` with `modalities.input`, `limit.context`, `tool_call`, `reasoning`).
- Prices: bundled OpenGateway catalog (`opengateway-models.json`, regenerated by
  `packages/omo-opencode/scripts`) merged with live `client.model.list()` `cost` (input / output /
  cache read / cache write, USD per 1M tokens). Free only when every bucket is $0; unknown price is
  never free (conservative `strong_paid`).
- Prices are NOT hardcoded: the live `cost` overrides the bundled snapshot at dispatch.

### Enabled-state source (authoritative permission boundary)

`packages/omo-opencode/src/shared/model-enable-state.ts` reduces the OpenCode config (read at runtime
via `client.config.get()`) into `disabledProviders` / `enabledProviders` / per-provider
`whitelist` / `blacklist` / per-model `status`. A model is enabled only when its provider is not
disabled (and is inside `enabled_providers` when that allowlist is set) and the model is neither
blacklisted nor marked `disabled`, and passes the provider whitelist. `filterEnabledModelKeys` drops
disabled models before the candidate ladder; `disabledModels` are also merged into the ladder's
`unavailable` set so a disabled user-configured resolved model is excluded too. We never modify user
configuration and never re-enable a disabled model.

### Candidate ranking

free → stronger/alternate free → cheap paid (input ≤ $1/1M) → strong paid → MAIN's own model as the
terminal `expert` rung. Within a tier, cheapest input price first. Disabled and negative-cached models
are skipped; escalation is forward-only.

### Capability filtering

`buildDelegationWorkerCandidates` accepts an optional `required` capability requirement
(`vision`, `tool_call`, `reasoning`, `min_context`) and a `modelInfo` metadata map. Candidates lacking
a known required capability are dropped (a screenshot task selects vision-capable candidates only,
tool-using tasks select tool-capable candidates only, long-context tasks select sufficient-context
models). Metadata (vision / tool_call / reasoning / context_limit) and all four price dimensions are
attached to each `WorkerCandidate` so the Resource Governor can reason economically.

### Changes

- `features/delegation-ladder/types.ts`: extend `WorkerCandidate` with output/cache price dimensions
  plus `context_limit` / `vision` / `tool_call` / `reasoning` (backward compatible).
- `features/delegation-first/free-worker-candidates.ts`: `ModelCapabilityInfo`,
  `WorkerCapabilityRequirement`, `modelInfo` + `required` inputs, capability filtering, full price +
  metadata attachment.
- `shared/model-enable-state.ts` (new): `computeModelEnableState`, `isModelEnabled`,
  `filterEnabledModelKeys`.
- `tools/delegate-task/available-models.ts`: `getModelsWithPricingAndMetadataForDelegateTask`
  (capability/context metadata) + `getEnabledModelState` (live config reduce).
- `tools/delegate-task/tools.ts` + `background-task.ts`: reduce enabled state, filter the candidate
  `available` set, merge `disabledModels` into `unavailable`, pass `modelInfo`.

### Tests (all green)

`model-enable-state` (7: disabled providers, enabled allowslist, blacklist, whitelist, per-model
status, filtering, empty config), `free-worker-candidates-capability` (6: four price dims retained,
vision requirement, min_context, metadata attach, unknown passthrough, tool_call requirement).
Existing `free-worker-candidates` + `free-worker-candidates-routing` (9) remain green. Regression:
`delegation-first` + `delegate-task` 585 pass, `delegation-ladder` + `resource-governor` 121 pass.
`tsgo --noEmit` clean; `bun run build` succeeds; `dist/index.js` carries `computeModelEnableState`,
`filterEnabledModelKeys`, `isModelEnabled`, `buildDelegationWorkerCandidates`,
`getModelsWithPricingAndMetadataForDelegateTask`, `getEnabledModelState`; smoke-import OK (`default` +
`omoPlugin`).

### Live validation

SKIPPED — this environment has no live OpenCode gateway and no provider credentials to drive a real
session; a real free worker could not be launched without risking paid usage or fabricating evidence.
Unit + integration coverage (585 delegated/545 total in-scope tests) is the honest evidence boundary
here. The live pool is discovered at runtime, so the counts called for in the goal will resolve only
inside a real workspace. No cost was incurred.

---

## Model Routing: usable-worker selection + disabled-model hard-fail (previous change)
