...
| Simple work | `simple-work` | Small, well-specified edits where speed and cost matter | `deepseek/deepseek-v4-flash (max)` -> `openai\|openai-codex/gpt-5.6-luna-fast (low)` -> `anthropic\|github-copilot/claude-haiku-4-5` |
| Deep work | `deep-work` | Hard problems that need maximum reasoning; the `deep` category chain verbatim | `deepseek\|opencode-go/deepseek-v4-flash (max)` -> `openai\|openai-codex\|github-copilot\|opencode/gpt-6-astra (high)` -> same providers `/gpt-5.6-sol (medium)` |
...
| --- | --- | --- | --- |
| `explore` | Fast codebase grep and pattern discovery | `deepseek-v4-flash` (max) | `deepseek/deepseek-v4-flash (max)` -> `openai\|openai-codex/gpt-5.6-luna-fast (low)` -> `opencode-go\|bailian-coding-plan/qwen3.7-plus` -> cheaper utility rungs -> `anthropic\|github-copilot/claude-haiku-4-5` -> `openai\|openai-codex/gpt-5.4-nano` |
| `librarian` | Documentation and OSS code search | `deepseek-v4-flash` (max) | Same chain as `explore`. |
...
1. **Match the family to the role.** Claude-family models fit the communicative roles: the main agent and `plan-consultant`. GPT-family models fit `plan-reviewer`; `deep` / `ultrabrain` default to DeepSeek V4 Flash but keep GPT-6 Astra as their fallback rung.
...
| `ultrabrain` | Genuinely hard, logic-heavy tasks; goals only, no step-by-step | `opencode/deepseek-v4-flash (max)` | `deepseek\|opencode-go/deepseek-v4-flash (max)` -> `gpt-6-astra (max)` across `openai`, `openai-codex`, `github-copilot`, `opencode` -> `gpt-5.6-sol (max)` across the same providers |
| `deep` | 3D graphics, computer use, browser use, backend, algorithms, multimodal work, complex research | `opencode/deepseek-v4-flash (max)` | `deepseek\|opencode-go/deepseek-v4-flash (max)` -> `openai\|openai-codex\|github-copilot\|opencode/gpt-6-astra (high)` -> same providers `/gpt-5.6-sol (medium)` |
...
| `unspecified-low` | Doesn't fit elsewhere, low effort | `opencode/deepseek-v4-flash (max)` | `deepseek\|opencode-go/deepseek-v4-flash (max)` -> `xai\|github-copilot\|opencode/grok-4.6 (xhigh)` -> `gpt-5.6-terra (high)` -> `claude-sonnet-5 (low)` -> `qwen3.8-max-preview (max)` -> `xiaomi\|opencode-go/mimo-v2.5-pro (max)` |
| `unspecified-high` | Doesn't fit elsewhere, high effort | `opencode/deepseek-v4-flash (max)` | `deepseek\|opencode-go/deepseek-v4-flash (max)` -> `gpt-6-astra (high)` -> `claude-opus-5 (xhigh)` -> `zai-coding-plan\|opencode-go/glm-5.3 (max)` -> `kimi-k3 (max)` |
...
- **`deep` / `ultrabrain` on Claude or Kimi.** These categories default to DeepSeek V4 Flash and are built for an autonomous, reasoning-forward style. Other families finish eventually but don't shine.
